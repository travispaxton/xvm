...
val isCI            = System.getenv("CI") ?: "false"
val buildNum        = System.getenv("BUILD_NUMBER") ?: ""

if (isCI != "0" && isCI != "false" && buildNum.isNotEmpty()) {
...
val build = tasks.register("build") {
    ...
    // compile Ecstasy
    val coreSrc = fileTree(ecstasyMain).files.stream().
            mapToLong(File::lastModified).max().orElse(0)
    val coreDest = file(coreLib).lastModified()

    val turtleSrc = fileTree(turtleMain).files.stream().
            mapToLong(File::lastModified).max().orElse(0)
    ...
    val aggregateSrc = fileTree(aggregateMain).files.stream().
            mapToLong(File::lastModified).max().orElse(0)
    ...
    val collectionsSrc = fileTree(collectionsMain).files.stream().
            mapToLong(File::lastModified).max().orElse(0)
    ...
    val cryptoSrc = fileTree(cryptoMain).files.stream().
            mapToLong(File::lastModified).max().orElse(0)
    ...
    val netSrc = fileTree(netMain).files.stream().
            mapToLong(File::lastModified).max().orElse(0)
    ...
    val jsonSrc = fileTree(jsonMain).files.stream().
            mapToLong(File::lastModified).max().orElse(0)
    ...
    val oodbSrc = fileTree(oodbMain).files.stream().
            mapToLong(File::lastModified).max().orElse(0)
    ...
    val imdbSrc = fileTree(imdbMain).files.stream().
            mapToLong(File::lastModified).max().orElse(0)
    ...
    val jsondbSrc = fileTree(jsondbMain).files.stream().
    mapToLong(File::lastModified).max().orElse(0)
    ...
    val webSrc = fileTree(webMain).files.stream().
            mapToLong(File::lastModified).max().orElse(0)
    ...
    val xeniaSrc = fileTree(xeniaMain).files.stream().
            mapToLong(File::lastModified).max().orElse(0)
    ...
    val bridgeSrc = fileTree(bridgeMain).files.stream().
            mapToLong(File::lastModified).max().orElse(0)
...
tasks.register("dist-local") {
    ...
    doLast {
        ...
        if (xecLink.isNotEmpty()) {
            val xecFile    = Paths.get(xecLink).toRealPath()
            val libexecDir = file("$xecFile/../..")
            var updated    = false

            val srcBin = fileTree(binDir).files.stream().
                    mapToLong(File::lastModified).max().orElse(0)
            val dstBin = fileTree("$libexecDir/bin").files.stream().
                    mapToLong(File::lastModified).max().orElse(0)
            if (srcBin > dstBin) {
                copy {
                    from("$binDir/")
            ...
            val srcLib = fileTree("$libDir/").files.stream().
                    mapToLong(File::lastModified).max().orElse(0)
            val dstLib = fileTree("$libexecDir/lib").files.stream().
                    mapToLong(File::lastModified).max().orElse(0)
            if (srcLib > dstLib) {
                copy {
                    from("$libDir/")
            ...
            val srcJts = fileTree("$javaDir/").files.stream().
                    mapToLong(File::lastModified).max().orElse(0)
            val dstJts = fileTree("$libexecDir/javatools").files.stream().
                    mapToLong(File::lastModified).max().orElse(0)
            if (srcJts > dstJts) {
                copy {
                    from("$javaDir/")
...
val distEXE = tasks.register("distEXE") {
    ...
    doLast {
        ...
        if (output.toString().trim().isNotEmpty()) {
            // notes:
